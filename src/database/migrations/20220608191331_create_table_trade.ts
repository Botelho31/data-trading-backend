...
export async function up (knex: Knex): Promise<void> {
  return knex.schema.createTable('trade', function (table) {
    ...
    table.string('saleFrom').references('public_address').inTable('user').onDelete('CASCADE')
    table.string('saleTo').references('public_address').inTable('user').onDelete('CASCADE')
